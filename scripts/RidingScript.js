...
class Ridingmanager {
	...
	static async UnsetRidingHeight(pRiderTokens, pRiddenTokens) {} //Reduces Tokens Elevation by Riding height or sets it to the height of the previously ridden token
				...
				if (pRidden) {
					if (pRidden.isOwner) {
						...
						if (pChanges.hasOwnProperty("elevation")) {
							if (vChange.hasOwnProperty("elevation")) {
								vztarget = pRidden.elevation + vChange.elevation;
							}
							else {
								vztarget = pRidden.elevation + (pChanges.elevation - pToken.elevation);
							}
						}
						/*
	...
	static async planRiderTokens(pRiddenToken, pChanges, pRiderTokenList, pAnimations = true) {
		...
		//Grappled riders always use "Following" placement: they stay wherever they currently are
		//when first grappled, and are moved by FollowingManager.OnTokenmove (FollowingScript.js)
		//whenever the grappler itself moves (driven by the "moveToken" hook).
	}
