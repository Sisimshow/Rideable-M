...
class RideableSheetSettings {
	...
	static FixSheetWindow(pHTML, pIndentifier) {
		...
		if (game.release.generation > 12) {
			const nav = pHTML.querySelector(pIndentifier);
			// Only apply styles once to avoid resetting on each render
			if (!nav.dataset.rideableFixed) {
				nav.dataset.rideableFixed = "true";
