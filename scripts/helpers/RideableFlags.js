...
class RideableFlags {
	...
	static #GrapplePlacementFlag (pToken) {
		//Grapple placement is always "Following"
		return "Following";
	}
	...
	static #RiderOffsetFlag (pToken) {
		//returns content of RiderOffsetFlag of pToken (if any) (array of numbers)
		let vFlag = this.#RideableFlags(pToken);
		
		if (vFlag) {
			if (vFlag.hasOwnProperty(cRiderOffsetF)) {
				return vFlag.RiderOffsetFlag;
			}
		}
	...
	static RidersOffset(pObject, pSceneScales = false) {
		...
		let offset = this.#RiderOffsetFlag(pObject);
		
		for (let i = 0; i <= 1; i++) {
			if (offset[i] == undefined) offset[i] = 0;
			
			if (pSceneScales) {
				offset[i] = cScale * offset[i];
			}
		}
	...
	static async startFollowing(pFollower, pToken, pDistance, pPlayerID = game.userId) {
		await this.#setfollowedTokenFlag(pFollower, pToken.id);

		await this.#setfollowDistanceFlag(pFollower, pDistance);

		await this.#setFollowOrderPlayerIDFlag(pFollower, pPlayerID);
	}
